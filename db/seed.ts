...
const CITY_COORDS = {
  "Delhi":     [28.6139, 77.2090], "Mumbai":    [19.0760, 72.8777], "Chennai":   [13.0827, 80.2707], "Bangalore": [12.9716, 77.5946],
...
const HUBS = [
  ...
  // New hubs matching expanded ROAD_GRAPH
...
const generateSyntheticPredictions = () => {
    console.log("Generating synthetic records...");
    let preds = [];
    const categories = ["none", "minor", "major", "critical"];

    for (let i = 0; i < 500; i++) {
        const distance_km = Math.random() * (1500 - 10) + 10;
        const weather_score = Math.floor(Math.random() * 4);
        const traffic_score = Math.floor(Math.random() * 4);
        const time_of_day = Math.floor(Math.random() * 24);
        const day_of_week = Math.floor(Math.random() * 7);
        const weight_tons = Math.random() * (30 - 1) + 1;
        const hub_load_pct = Math.random() * (1.0 - 0.1) + 0.1;

        let effective_traffic = traffic_score;
        if ((time_of_day >= 22 && time_of_day <= 23) || (time_of_day >= 0 && time_of_day <= 5)) {
            effective_traffic = Math.max(0, traffic_score - 1);
        }

        let delay_idx = 0;
        if (weather_score >= 2 && effective_traffic >= 2) {
            delay_idx = Math.random() < 0.5 ? 2 : 3;
        } else if (weather_score === 3) {
            delay_idx = Math.random() < 0.5 ? 2 : 3;
        } else if (effective_traffic === 3) {
            delay_idx = Math.random() < 0.5 ? 2 : 3;
        } else if (hub_load_pct > 0.9) {
            const r = Math.random();
            let base_idx = r < 0.4 ? 0 : r < 0.8 ? 1 : 2;
            delay_idx = Math.min(3, base_idx + 1);
        } else {
            const r = Math.random();
            delay_idx = r < 0.6 ? 0 : r < 0.85 ? 1 : r < 0.95 ? 2 : 3;
        }

        const features = {
            distance_km, weather_score, traffic_score, 
            time_of_day, day_of_week, weight_tons, hub_load_pct
        };
        ...
        preds.push({
            ...
            delay_category: categories[delay_idx],
...
async function main() {
    ...
    // Clear existing — order matters: delete child tables before parents
    await db.delete(predictions);
    await db.delete(routeHistories); // references trucks
    await db.delete(alerts);         // references trucks/hubs/packages
    await db.delete(packages);       // references hubs
    ...
    const syntheticPreds = generateSyntheticPredictions();
    
    // Insert in chunks to avoid query length limits
    console.log("Inserting predictions...");
    for (let i = 0; i < syntheticPreds.length; i += 100) {
        await db.insert(predictions).values(syntheticPreds.slice(i, i + 100));
    }
